...
class CommandThread:
    def __init__(self, pos_data_cb, meas_data_cb) -> None:
        self.queue = queue.Queue()
        self.pos_data_cb = pos_data_cb
        self.meas_data_cb = meas_data_cb
    ...
    def run(self):
        ...
        with SyncCrazyflie(URI, cf=self.cf) as scf:
            logconf_batt = LogConfig(name='Battery Levels', period_in_ms=100)
            logconf_batt.add_variable('pm.batteryLevel', 'uint8_t')
            logconf_batt.add_variable('pm.vbat', 'float')
            scf.cf.log.add_config(logconf_batt)
            logconf_batt.data_received_cb.add_callback(log_batt_callback)
            logconf_batt.start()
            with MotionCommander(scf) as motion_commander:
                with Multiranger(scf) as multiranger:
                    ...
                    while keep_flying:
                        hover_command = self.queue.get()
                        if 'quit' in hover_command:
                            keep_flying = False
                            continue
                        motion_commander.start_linear_motion(hover_command['x'], hover_command['y'],  0.0, hover_command['yaw'])
    ...
    def connected(self, URI):
        ...
        try:
            ...
            lpos.data_received_cb.add_callback(self.pos_data_cb)
            lpos.start()
        ...
        try:
            ...
            lmeas.data_received_cb.add_callback(self.meas_data_cb)
            lmeas.start()
        except KeyError as e:
            print('Could not start log configuration,'
                  '{} not found in TOC'.format(str(e)))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, URI):
        ...
        self.comms_thread = CommandThread(self.pos_data, self.meas_data)
        self.handle = threading.Thread(target=self.comms_thread.run)
        self.handle.start()
    ...
    def sendHoverCommand(self):
        self.comms_thread.queue.put_nowait(self.hover)
    ...
    def on_key_press(self, event):
        if (not event.native.isAutoRepeat()):
            ...
            if (event.native.key() == QtCore.Qt.Key_S):
                self.keyCB('height', -0.1)
            if (event.native.key() == QtCore.Qt.Key_Q):
                self.keyCB('quit', 0)
